...
function insertSpeedtestUser($userId, $ip, $ispinfo, $extra, $ua, $lang, $dl, $ul, $ping, $jitter, $log, $returnExceptionOnError = false)
{
    ...
    try {
        ...
        (user_id,ip,ispinfo,extra,ua,lang,dl,ul,ping,jitter,log)
        VALUES (?,?,?,?,?,?,?,?,?,?,?)'
        );
        $stmt->execute([
...
function getSpeedtestUserById($id,$returnExceptionOnError = false)
{
    ...
    try {
        $stmt = $pdo->prepare(
            'SELECT
            id, user_id, timestamp, ip, ispinfo, ua, lang, dl, ul, ping, jitter, log, extra
...
function getLatestSpeedtestUsers()
{
    ...
    try {
		...
		$sql .= ' id, user_id, timestamp, ip, ispinfo, ua, lang, dl, ul, ping, jitter, log, extra
            FROM speedtest_users
