	...
	interval = setInterval(
		function() {
			...
			if (!graceTimeDone) {
				...
				//update status
				dlStatus = ((speed * settings.overheadCompensationFactor) / (settings.useMebibits ? 8 * 1048576 : 8 * 1000000)).toFixed(2); // speed is multiplied by overhead compensation, then divided by 8 to go from megabits to megabytes
		...
		interval = setInterval(
			function() {
				...
				if (!graceTimeDone) {
					...
					//update status
					ulStatus = ((speed * settings.overheadCompensationFactor) / (settings.useMebibits ? 8 * 1048576 : 8 * 1000000)).toFixed(2); // speed is multiplied by overhead compensation, then divided by 8 to go from megabits to megabytes
